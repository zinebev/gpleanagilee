...
const statutTone: Record<Tache["statut"], string> = {
      ...
      {showForm && (
        <div className="mt-4 rounded-lg border border-hairline bg-card p-5">
          ...
          <div className="mt-3 flex items-center gap-2">
            <label className="text-sm text-graphite-500">Priorité</label>
      ...
      <div className="mt-6 flex flex-col gap-2">
        {taches.map((t) => (
          <div key={t.id} className="flex items-center gap-4 rounded-lg border border-hairline bg-card p-4">
            <div className="flex flex-col items-center" onPointerDown={(e) => e.stopPropagation()}>
              <button onClick={() => changePriorite(t, (t.priorite ?? 1) - 1)} className="text-graphite-400 hover:text-graphite-700" aria-label="Monter la priorité">▲</button>
              <span className="tnum text-sm font-semibold text-graphite-700">{t.priorite ?? 1}</span>
              <button onClick={() => changePriorite(t, (t.priorite ?? 1) + 1)} className="text-graphite-400 hover:text-graphite-700" aria-label="Baisser la priorité">▼</button>
            </div>
